...
export default function HomePage() {
    ...
    const getFilmData = async (value, endpoint, query, page, array) => {
        const response = await fetch(`https://api.themoviedb.org/3/${endpoint}?api_key=${API_KEY}${query ? query : ''}&include_adult=false&language=${language}-${language}&page=${page || '1'}`)
        const { results } = await response.json()
        array ? value([...array, ...results]) : value(results)
    }
